...
playLocalhost0 parentPlayAttrs taskAttrs taskVars = do
  ...
  debugCallTaskInALoop0 <- traverseLoop (\__haxible_loop_item ->  runTask playAttrs "debug" ([("debug", [json|{"msg":"loop {{ item }}"}|]), ("name", [json|"Call task in a loop"|])] <> taskAttrs) ([("item", __haxible_loop_item)] <> taskVars) )  [[json|"A"|], [json|"B"|], [json|"C"|]]
  debugLoopVar0 <- traverseLoop (\__haxible_loop_item ->  runTask playAttrs "debug" ([("debug", [json|{"msg":"loop control {{ lvar }}"}|]), ("name", [json|"Loop var"|])] <> taskAttrs) ([("lvar", __haxible_loop_item)] <> taskVars) )  [[json|"E"|]]
