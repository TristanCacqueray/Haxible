...
block0 parentPlayAttrs taskAttrs taskVars = do
  let playAttrs = parentPlayAttrs
      src = "test/playbooks"
  let when_ = False
  debug2 <- if when_ then (runTask src playAttrs "debug" ([("debug", [json|null|])] <> taskAttrs) (taskVars)) else pure [json|{"changed":false,"skip_reason":"Conditional result was False"}|]
  pure $ [debug2]
...
tasksTasksPrintYaml1 parentPlayAttrs taskAttrs taskVars = do
  ...
  debug1 <- runTask src playAttrs "debug" ([("debug", [json|{"msg":"Print task {{ print_arg }}"}|])] <> taskAttrs) (taskVars)
  pure $ [debug1]
...
tasksTasksPrintYaml0 parentPlayAttrs taskAttrs taskVars = do
  let playAttrs = parentPlayAttrs
      src = "test/playbooks/./tasks"
  debug0 <- runTask src playAttrs "debug" ([("debug", [json|{"msg":"Print task {{ print_arg }}"}|])] <> taskAttrs) (taskVars)
  pure $ [debug0]
