...
playLocalhost0 parentPlayAttrs taskAttrs taskVars = do
  let playAttrs = [("hosts", [json|"localhost"|]), ("vars", [json|{"play_var":"play-var"}|])] <> parentPlayAttrs
      src = "test/playbooks"
  factsSimpleFact0 <- extractFact <$> runTask src playAttrs "set_fact" ([("set_fact", [json|{"fact_var":"{{ play_var }}"}|]), ("name", [json|"Simple fact"|])] <> taskAttrs) (taskVars)
  factsExtraFact0 <- extractFact <$> runTask src playAttrs "set_fact" ([("set_fact", [json|{"cacheable":true,"extra_var":"extra-{{ fact_var }}"}|]), ("name", [json|"Extra fact"|])] <> taskAttrs) ([("fact_var", factsSimpleFact0)] <> taskVars)
  debug0 <- runTask src playAttrs "debug" ([("debug", [json|{"var":"extra_var"}|])] <> taskAttrs) ([("extra_var", factsExtraFact0)] <> taskVars)
